...
def list_kpis(db: Session = Depends(get_db), current_user: Vendedor = Depends(get_current_admin)):
    return get_all_kpis(db)


@router.get("/master-kpis/{kpi_id}", response_model=KpiMaestroResponse)
def get_kpi(kpi_id: int, db: Session = Depends(get_db), current_user: Vendedor = Depends(get_current_admin)):
...
def update_kpi_endpoint(kpi_id: int, data: KpiMaestroUpdate, db: Session = Depends(get_db), current_user: Vendedor = Depends(get_current_admin)):
